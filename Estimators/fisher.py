...
def dimensionUniformSphere(py, alphas, number_of_data_points):
    '''
...
def SeparabilityAnalysis(X, ConditionalNumber=10, ProjectOnSphere=True, alphas=np.array([np.arange(.6, 1, .02)]),
                         ProducePlots=False, ncomp=False):
    '''
    ...
    npoints = len(X[:, 0])
    # Preprocess data
    Xp = preprocessing(X, True, True, True, ProjectOnSphere, ConditionalNumber=ConditionalNumber, ncomp=ncomp)
    # Check separability
    ...
    [n_alpha, n_single, alpha_single] = dimensionUniformSphere(py_mean, alphas, npoints)
    ...
    if ProducePlots:
        ...
        plt.xlabel('unseparability prob.p for \u03B1=%2.2f' % (alphas[0, alpha_ind_selected]), fontsize=16)
        ...
        if True in np.isnan(n_alpha):
            plt.semilogy(alphas[0, :np.where(np.isnan(n_alpha))[0][0]], py_mean[:np.where(np.isnan(n_alpha))[0][0]],
                         'bo-',
                         'LineWidth', 3)
        else:
        ...
        plt.xlabel('\u03B1')
        plt.ylabel('Mean unseparability prob.', fontsize=16)
